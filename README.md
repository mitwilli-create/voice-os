...
Every stage has a deterministic offline implementation, so scoring and gating are reproducible without an API key. With live routing enabled, Voice OS tries its approved Claude Code route and sandboxed Codex subscription seats before any explicitly allowed metered application programming interface. Antigravity and Grok command-line interfaces are not Voice OS adapters because their prompt transport, clean working directory, tool and configuration isolation, nonpersistent sessions, and child-environment isolation have not all been proven here. A degraded route may run only after a retryable refusal, malformed response, credential, quota, capacity, timeout, or availability failure when `VOICE_OS_ALLOW_DEGRADED=true` and `VOICE_OS_ALLOWED_PROVIDERS` explicitly includes it. The default allowlist contains only the approved subscription adapters. An unknown alternate hard-stops before draft or exemplar text is sent.

**Privacy note:** live mode sends the draft text, target profile, banned phrases, revision signals, selected real exemplar messages (bounded, held-in only), and distilled knowledge-base voice patterns to each attempted policy-approved provider. `VOICE_OS_PROVIDER` and `VOICE_OS_MODEL` name the requested route but never move a metered route ahead of an eligible subscription seat or bypass the policy gate. Cross-provider or lower-capability fallback requires `VOICE_OS_ALLOW_DEGRADED=true` and an explicit provider entry in `VOICE_OS_ALLOWED_PROVIDERS`. Set `VOICE_OS_OFFLINE=1` to stop all provider calls before client caches and routing. Live envelopes and checkpoints stamp the requested slot, resolved model, account type, policy outcome, and a bounded prompt-free ledger for every failed attempt. Checkpoints contain draft and exemplar text; they live under the gitignored `var/` directory by default, or wherever `var_dir` / `VOICE_OS_VAR_DIR` points, so keep overrides outside tracked paths.
